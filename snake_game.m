...
counter = 0;
loseMsg = sprintf('You have lost, Score:%d',counter); %Displays the score after you lose
difficulty = 0;
rate=0.05;
...
%a one is substracted from axis limit to make sure that is the snake
%doesn't spawn on the border in boundaries mode
...
figure('KeyPressFcn',@pressed_btn);
    function pressed_btn(~,event)%pressed button function for movement
...
while (ex~=1)%runs the snake as long as q is not pressed
    ...
    %snake movement for the snake body only
    size_snake=size(snake);
    size_snake=size_snake(1);
    for j=size_snake+ate:-1:2
        snake(j,:)=snake(j-1,:);
    end
    
    %generating the new coordinate for snake's head only
   ...
   draw_snake(snake,food,size_snake,axis_limit, trap, counter)
...
function draw_snake(snake,food,size_snake,axis_limit, trap, counter)
    %plots the snake
      for p = 1:size_snake
          plot(snake(p,1),snake(p,2), 'wo')
          hold on
      end
      plot(food(1,1),food(1,2), 'pr')%plots the food
      plot(trap(1,1),trap(1,2), 'md')% plots the trap
      ...
      set(scoreBox,'String',int2str(counter))
